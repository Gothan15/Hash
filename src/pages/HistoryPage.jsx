...
function getScanStatusDisplay(scanResult) {
  ...
  if (infestados > 0) {
    ...
    return <Badge variant="outline" className="bg-yellow-50 text-yellow-700 hover:bg-yellow-50">Sospechoso</Badge>
  } else if (infestados === 0 && sospechosos === 0) {
    return <Badge variant="outline" className="bg-green-50 text-green-700 hover:bg-green-50">Limpio</Badge>
  }
  return "-"
}
...
function getVirusTotalStatusDisplay(vtResult) {
  ...
  if (result.stats) {
    ...
    if (malicioso > 0) {
      ...
      return <Badge variant="outline" className="bg-yellow-50 text-yellow-700 hover:bg-yellow-50">Sospechoso ({sospechoso})</Badge>
    } else if (limpio >= 0) {
      return <Badge variant="outline" className="bg-green-50 text-green-700 hover:bg-green-50">Limpio ({limpio})</Badge>
    }
  }
  return "-"
}
  ...
  return (
    <Card className="w-full mx-auto overflow-x-auto">
      <CardHeader>
        ...
        <CardDescription>Consulta el historial de todos los archivos analizados por su hash</CardDescription>
      </CardHeader>
        ...
        {isLoading ? (
          ...
          <div className="overflow-x-auto w-full">
            <Table className="min-w-[600px] text-xs md:text-sm">
              <TableHeader>
                <TableRow>
                  <TableHead className="break-all max-w-[180px] md:max-w-none">Hash (SHA-256)</TableHead>
              ...
              <TableBody>
                {hashes.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={3} className="text-center">No hay historial disponible</TableCell>
                  </TableRow>
                ) : (
                  hashes.map((hash) => {
                    ...
                    if (items.length > 10) {
                      ...
                      return (
                        <TableRow key={hash}>
                          <TableCell className="font-mono text-xs break-all max-w-[180px] md:max-w-none">{hash}</TableCell>
                          <TableCell>{items.length}</TableCell>
                          <TableCell>
                            <button className="underline text-blue-600" onClick={() => { setDialogItems(items); setDialogHash(hash); setDialogOpen(true) }}>Ver historial</button>
                          </TableCell>
                        </TableRow>
                      )
                    }
                            ...
                            <AccordionItem value={hash}>
                              <AccordionTrigger>
                                <div className="flex flex-col w-full">
                                  <span className="font-mono text-xs break-all max-w-[180px] md:max-w-none">{hash}</span>
                                  <span className="text-xs text-muted-foreground">{items.length} análisis</span>
                                </div>
                              </AccordionTrigger>
                              <AccordionContent>
                                <div className="overflow-x-auto w-full">
                                  <Table className="min-w-[500px] text-xs md:text-sm">
                                    <TableHeader>
                                      <TableRow>
                                        <TableHead className="cursor-pointer select-none">Nombre {sortConfig.key === "name" ? (sortConfig.direction === "asc" ? "▲" : "▼") : null}</TableHead>
                                        <TableHead className="cursor-pointer select-none">Fecha {sortConfig.key === "createdAt" ? (sortConfig.direction === "asc" ? "▲" : "▼") : null}</TableHead>
                                    ...
                                    <TableBody>
                                      {sortItems(items).map((item, idx) => (
                                        <TableRow key={item.createdAt + idx}>
                                          <TableCell className="break-all max-w-[120px] md:max-w-none">{item.name || "-"}</TableCell>
                                          <TableCell>{formatDate(item.createdAt)}</TableCell>
                                          <TableCell>{getScanStatusDisplay(item.scanResult)}</TableCell>
                                          <TableCell>{getVirusTotalStatusDisplay(item.virusTotalResult)}</TableCell>
                                        </TableRow>
                                      ))}
                                    </TableBody>
                                  </Table>
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              <DialogContent className="max-w-2xl w-full p-2 sm:p-6">
                <DialogHeader>
                  ...
                  <DialogDescription className="font-mono text-xs break-all">{dialogHash}</DialogDescription>
                </DialogHeader>
                ...
                <DialogFooter>
                  <button className="btn w-full sm:w-auto" onClick={() => setDialogOpen(false)}>Cerrar</button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
          </div>
